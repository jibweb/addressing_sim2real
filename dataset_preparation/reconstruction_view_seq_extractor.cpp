...
main(int argc, char* argv[]) {
  ...
  **************************************************************************/
  ...
  bool full_object_mode = true;

  for (uint sp = 0; sp <= sphere_positions.size(); sp++) {
    // Creating directory
    std::string track_dir = output + "track_" + std::to_string(sp) + "/";

    // On the last pass, we go through all sphere position to get the full object
    if (sp == sphere_positions.size() || full_object_mode) {
      track_dir = output + "full_object/";
      full_object_mode = true;
      sphere_positions = dmr.createSphere(sphere_distance, 2);
      pose_per_traj = sphere_positions.size();
    }
    ...
    * Compute camera pose
    *************************************************************************/
    Eigen::Vector3f position, n_pos, traj_vec;

    if (!full_object_mode) {
      position = sphere_positions[sp];
      ...
      traj_vec = n_pos.cross(z);
      traj_vec.normalize();
    }

    for (uint pos_idx=0; pos_idx < pose_per_traj; pos_idx++) {
      Eigen::Vector3f real_pos;

      if (!full_object_mode) {
        real_pos = position + (static_cast<float>(pos_idx) - 30.)/15.*traj_vec;
      } else {
        real_pos = sphere_positions[pos_idx];
      }
